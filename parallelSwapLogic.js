...
function calculate_dx_float(mu, pool,inputToken) {
    let p = formatPool(pool, inputToken);
...
function solveForMuFloat(pools, totalDeltaX, inputToken) {
    if (pools.length > 0){
...
function getAllocationsAndOutputs(totalInput, filteredPoolStructList,inputToken) {
 ...
 function calculateOptimalOutput(pools, inputAmount, inputToken) {
	...
	let mu = solveForMuFloat(pools, inputAmount, inputToken);
